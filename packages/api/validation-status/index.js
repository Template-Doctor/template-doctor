...
module.exports = async function (context, req) {
    ...
    try {
        ...
        let runId =
            req.query.runId || req.params?.runId || req.query.localRunId;
        ...
        try {
            ...
            context.log(
                `validation-status: GitHub client mode: ${token ? "authenticated" : "unauthenticated"} (octokit)`,
            );
        } catch (e) {
            context.log.warn(
                `validation-status: @octokit/rest not available, using fetch fallback. ${e && e.message ? e.message : e}`,
            );
        }
        ...
        async function ghFetch(path, init = {}) {
            ...
            if (!res.ok) {
                ...
                const err = new Error(
                    `GitHub ${res.status} ${res.statusText}: ${t}`,
                );
        ...
        if (!githubRunId) {
            ...
            async function tryDiscover(currentOctokit) {
                ...
                try {
                    ...
                    if (currentOctokit) {
                        const runsResp =
                            await currentOctokit.actions.listWorkflowRuns({
                                owner,
                                repo,
                        ...
                        const data = await ghFetch(
                            `/repos/${owner}/${repo}/actions/workflows/${encodeURIComponent(workflowFile)}/runs?branch=${encodeURIComponent(branch)}&event=workflow_dispatch&per_page=100`,
                        );
                    ...
                    for (const r of candidates) {
                        ...
                        const commitMsg =
                            r.head_commit && r.head_commit.message
                                ? String(r.head_commit.message)
                                : "";
                        if (
                            (title && String(title).includes(runId)) ||
                            commitMsg.includes(runId)
                        ) {
                            return r;
                        }
                    }
                    ...
                    if (
                        wfErr &&
                        (wfErr.status === 401 ||
                            /bad credentials/i.test(wfErr.message))
                    ) {
                        context.log.warn(
                            `listWorkflowRuns failed for ${workflowFile} with 401; will retry unauthenticated.`,
                        );
                        throw Object.assign(new Error("retry-unauth"), {
                            code: "RETRY_UNAUTH",
                        });
                    }
                    context.log.warn(
                        `listWorkflowRuns failed for ${workflowFile}: ${wfErr.message}`,
                    );
                }
                ...
                try {
                    ...
                    if (currentOctokit) {
                        const repoRuns =
                            await currentOctokit.actions.listWorkflowRunsForRepo(
                                {
                                    owner,
                                    repo,
                                    per_page: 100,
                        ...
                        const data = await ghFetch(
                            `/repos/${owner}/${repo}/actions/runs?per_page=100&branch=${encodeURIComponent(branch)}&event=workflow_dispatch`,
                        );
                        candidates = data.workflow_runs || [];
                    }
                    inspected += candidates.length;
                    ...
                    if (
                        repoErr &&
                        (repoErr.status === 401 ||
                            /bad credentials/i.test(repoErr.message))
                    ) {
                        context.log.warn(
                            `listWorkflowRunsForRepo failed with 401; will retry unauthenticated.`,
                        );
                        throw Object.assign(new Error("retry-unauth"), {
                            code: "RETRY_UNAUTH",
                        });
                    ...
                    context.log.warn(
                        `listWorkflowRunsForRepo failed: ${repoErr.message}`,
                    );
                }
                return null;
            }
            ...
            try {
                ...
                if (e && e.code === "RETRY_UNAUTH") {
                    ...
                    context.log.warn(
                        "Falling back to unauthenticated GitHub API client due to bad credentials.",
                    );
            ...
            if (discoveredRun) {
                ...
                context.log(
                    `validation-status: no matching workflow run found for runId ${runId} after inspecting ${inspected} runs; returning pending.`,
                );
                context.res = {
                    status: 200,
                    headers: corsHeaders(),
        ...
        try {
            if (octokit) {
                ...
                ghData = await ghFetch(
                    `/repos/${owner}/${repo}/actions/runs/${Number(githubRunId)}`,
                );
            }
        } catch (getErr) {
            ...
            if (
                ...
                const hint =
                    "Private repo access requires a valid GH_WORKFLOW_TOKEN with repo and workflow scopes (or fine-grained: Actions Read, Contents Read, Metadata Read) and SAML SSO authorization if your org enforces it.";
                context.log.warn(
        ...
        const wantArchive =
            req.query.includeLogsUrl === "1" ||
            req.query.includeLogsUrl === "true";
        const wantJobLogs =
            req.query.includeJobLogs === "1" ||
            req.query.includeJobLogs === "true";
            ...
            try {
                ...
                if (wantArchive) {
                    ...
                    const res = await fetch(
                        `https://api.github.com/repos/${owner}/${repo}/actions/runs/${Number(githubRunId)}/logs`,
                        {
                            headers: baseHeaders,
                            redirect: "manual",
                        },
                ...
                if (wantJobLogs) {
                    ...
                    if (octokit) {
                        ...
                        jobsData = await ghFetch(
                            `/repos/${owner}/${repo}/actions/runs/${Number(githubRunId)}/jobs?per_page=100`,
                        );
                    }
                    ...
                    for (const j of jobs) {
                        const res = await fetch(
                            `https://api.github.com/repos/${owner}/${repo}/actions/jobs/${j.id}/logs`,
                            {
                                headers: baseHeaders,
                                redirect: "manual",
                            },
        ...
        const isGitHubError =
            err.status ||
            (err.message && err.message.toLowerCase().includes("github"));
