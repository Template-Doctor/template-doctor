...
function showWarning(t:string,m:string){ const n=notify(); if(n?.showWarning) n.showWarning(t,m,5000); else console.warn(t+': '+m);}
function showSuccess(t:string,m:string){ const n=notify(); if(n?.showSuccess) n.showSuccess(t,m,3000); else console.log(t+': '+m);}
  ...
  try {
    ...
    if(existing.length){
      ...
      if(matching.length){
        ...
        const shouldResume = confirm(`Found ${matching.length} previously scanned repositories. Resume and skip successful scans?`);
        if(shouldResume){
          resumeMode = true;
        } else {
          await clearProgress();
    ...
    if($(progressTextId)) ($(progressTextId) as HTMLElement).textContent = `0/${repos.length} Completed`;
...
async function processOneRepo(url: string, index: number, existing?: BatchProgressEntry){
  ...
  try {
    ...
    item.querySelector('.batch-item-message')!.textContent = `Analysis complete: ${issues} issues, ${passed} passed`;
