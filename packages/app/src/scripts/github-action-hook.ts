...
async function submitAnalysisToGitHub(result: AnalysisResult, username: string): Promise<SubmitResult> {
  ...
  console.log('[submitAnalysisToGitHub] Starting submission:', { repoUrl: result.repoUrl, username, timestamp: result.timestamp });
  ...
  console.log('[submitAnalysisToGitHub] API config:', { apiBase, dispatchTargetRepo: cfg.dispatchTargetRepo || 'not set' });
  ...
  async function post(url: string){
    ...
    console.error('[submitAnalysisToGitHub] Error response:', { url, status: resp.status, text: text.slice(0,200) });
  ...
  if (primary.status === 404 && !cfg.forceLegacyDispatchPath){
    ...
    if (legacy.status === 404) {
      ...
      return { success:false, error:'Endpoint not found (404): ensure Function deployed (route v4/submit-analysis-dispatch) or set forceLegacyDispatchPath=true.' };
    }
