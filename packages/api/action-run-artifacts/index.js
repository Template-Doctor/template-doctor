...
async function getArtifactsList(workflowOwner, workflowRepo, workflowRunId) {
...
async function getArtifactsForRun(workflowOwner, workflowRepo, workflowRunId) {

  const rawResponse = await getArtifactsList(workflowOwner, workflowRepo, workflowRunId)
...
module.exports = async function (context, req) {
  ...
  try {
    ...
    const artifactsData = await getArtifactsForRun(workflowOwner, workflowRepo, workflowRunId);

    context.res = {
      status: 200,
      headers: { 'Access-Control-Allow-Origin': '*' },
      body: { error: null, data: artifactsData, context: { url: workflowUrl, uniqueInputId, ownerRepo: `${workflowOwner}/${workflowRepo}` } }
    };
  } catch (error) {
