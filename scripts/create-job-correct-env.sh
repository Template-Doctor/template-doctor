...
# API endpoint
# Resolve API base from environment or .env, defaulting to production host
...
echo "Starting a new container job with CORRECT environment variables..."
echo "Timestamp: $TIMESTAMP"
echo "Template: $TEMPLATE_REPO ($TEMPLATE_BRANCH)"

# Create job parameters with CORRECTED environment variable names
JSON_PAYLOAD=$(cat <<EOF
{
  ...
  "environment": {
    ...
    "AZD_ACTION": "init",
    "TD_RUN_ID": "$RUN_ID",
    "AZD_ENV_NAME": "test-env-$TIMESTAMP"
  }
}
EOF
)
