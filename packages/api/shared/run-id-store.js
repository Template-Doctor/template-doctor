...
if (!TOKEN) {
    console.warn(
        "Missing GH_WORKFLOW_TOKEN - using in-memory storage only (data will be lost on restart)",
    );
...
async function getAllRuns() {
    ...
    if (!TOKEN) {
        console.warn(
            "getAllRuns called without a token - using in-memory storage only",
        );
        return {
            sha: null,
            data: {},
    ...
    try {
        ...
        if (!res.ok) {
            ...
            try {
                ...
                console.error(
                    `GitHub API error details: ${JSON.stringify(errorJson)}`,
                );

                // Check for specific error conditions
                if (res.status === 401) {
                    throw new Error(
                        `GitHub authentication failed: Token is invalid or has expired - ${errorJson.message || ""}`,
                    );
                } else if (res.status === 403) {
                    throw new Error(
                        `GitHub permission denied: Token lacks required permissions - ${errorJson.message || ""}`,
                    );
                } else if (res.status === 404) {
                    throw new Error(
                        `GitHub resource not found: The repository or branch might not exist - ${errorJson.message || ""}`,
                    );
                }
            } catch (e) {
            ...
            throw new Error(
                `Failed to fetch ${FILE_PATH}: ${res.status} ${res.statusText} - ${responseText}`,
            );
        }
        ...
        try {
            ...
            console.error(
                `Failed to parse ${FILE_PATH} content: ${parseError.message}`,
            );
...
async function saveRuns(data, sha) {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            throw new Error(
                `Failed to save ${FILE_PATH}: ${res.status} ${res.statusText} - ${errorDetails}`,
            );
        }
        ...
        throw new Error(
            `GitHub storage operation failed: ${error.message}. Fallback storage used but data may be lost on restart.`,
        );
    }
}
