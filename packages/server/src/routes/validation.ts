 ...
 * @returns Markdown content of the validation result, or null if not found/error
 */
async function downloadValidationArtifact(
    ...
    try {
        ...
        if (!validationArtifact) {
            console.log('No validation result artifact found', { runId });
            return null;
        }
        ...
        // 4. Extract result file from ZIP
        const buffer = Buffer.from(await downloadResponse.arrayBuffer());
        const zip = new AdmZip(buffer);
        const zipEntries = zip.getEntries();

        // Find the result file - try .md first, then .log, then any text file
        let resultEntry = zipEntries.find(
            (entry: any) => entry.entryName.endsWith('.md') && !entry.isDirectory
        );
        
        if (!resultEntry) {
            resultEntry = zipEntries.find(
                (entry: any) => entry.entryName.endsWith('.log') && !entry.isDirectory
            );
        }
        
        if (!resultEntry) {
            // Fallback: any non-directory entry
            resultEntry = zipEntries.find((entry: any) => !entry.isDirectory);
        }

        if (!resultEntry) {
            console.error('No result file found in artifact ZIP', {
                artifactId: validationArtifact.id,
                entries: zipEntries.map((e: any) => e.entryName),
            });
            return null;
        }

        const markdown = resultEntry.getData().toString('utf8');
        console.log('Successfully downloaded and extracted validation artifact', {
            runId,
            artifactId: validationArtifact.id,
        ...
        console.error('Error downloading artifact', { error, runId });
        return null;
    }
}

/**
 * Parses the validation result markdown file to extract AZD and PSRule status
 * @param markdown - Markdown content from validation result artifact
 * @returns Structured validation result data
 */
function parseAzdValidationResult(markdown: string): AzdValidationResult {
    // Parse AZD Up status (handle HTML tags like <b>azd up</b>)
    const azdUpSuccess =
        /- \[x\].*azd up/i.test(markdown) ||
        /:white_check_mark:.*azd up/i.test(markdown);

    // Parse AZD Down status (handle HTML tags like <b>azd down</b>)
