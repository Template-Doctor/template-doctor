...
function detectAuthenticationMethods(content: string): string[] {
  ...
  const patterns = [
    { name: 'Connection String', regex: /connectionString|ConnectionString/g },
    { name: 'Access Key', regex: /accessKey|AccessKey|accountKey|AccountKey/g },
    { name: 'SAS Token', regex: /sasToken|SasToken|sharedAccessSignature/g },
...
export async function runAnalyzer(
  ...
  const licenseFile = files.find(f => f.path && (f.path.toLowerCase() === 'license' || f.path.toLowerCase() === 'license.md'));
  if (licenseFile) {
    compliant.push({
      id: 'has-license',
      category: 'documentation',
  ...
  const azureYaml = files.find(f => f.path && (f.path.toLowerCase() === 'azure.yaml' || f.path.toLowerCase() === 'azure.yml'));
  if (azureYaml) {
    compliant.push({
      id: 'has-azure-yaml',
      category: 'azd',
  ...
  if (hasInfra) {
    ...
    if (bicepFiles.length === 0) {
      ...
      compliant.push({
        ...
        details: { count: bicepFiles.length, files: bicepFiles.map(f => f.path) }
      });
