...
async function getWorkflowRunData(
    workflowOwner,
    workflowRepo,
    workflowRunId,
    context = null,
    ...
    const rawResponse = await getWorkflowRun(
        workflowOwner,
        workflowRepo,
        workflowRunId,
        context,
    ...
    if (!rawResponse.ok) {
        ...
        throw new Error(
            `Trigger workflow failed: ${rawResponse.status} ${rawResponse.statusText} - ${errText}`,
        );
    }
    ...
    if (!data) {
        ...
        throw new Error(
            "Invalid response from GitHub when fetching workflow run data",
        );
    }
...
module.exports = async function (context, req) {
    ...
    try {
        ...
        if (!workflowOrgRep) {
            context.res = {
                status: 400,
                body: { error: "workflowOrgRep is required" },
            };
            return;
        }
        ...
        if (!incomingWorkflowOwner || !incomingWorkflowRepo) {
            context.res = {
                status: 400,
                body: { error: "workflowOrgRep must be in owner/repo format" },
            };
            return;
        }
        ...
        if (!workflowRunId) {
            context.res = {
                status: 400,
                body: { error: "workflowRunId is required" },
            };
            return;
        }
        ...
        context.res = {
            ...
            body: {
                ...
                context: { workflowOrgRep, workflowRunId },
            },
        };
    } catch (error) {
