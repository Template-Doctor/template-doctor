...
setup_mongodb() {
    ...
    case "$REPLY" in
        ...
        2)
            ...
            echo ""
            print_warning "${BOLD}CRITICAL:${NC} Production uses ${BOLD}MANAGED IDENTITY${NC}, NOT connection strings!"
            echo ""
            echo "How it works:"
            echo -e "  1. Container App gets System-Assigned Managed Identity automatically"
            echo -e "  2. Grant MI 'Cosmos DB Built-in Data Contributor' role on Cosmos DB"
            echo -e "  3. Application uses ${CYAN}COSMOS_ENDPOINT${NC} environment variable (NOT MONGODB_URI)"
            echo -e "  4. Code acquires token automatically via DefaultAzureCredential"
            echo -e "  5. Token is used as credentials in MongoDB connection string"
            echo ""
            echo "Setup options:"
            echo -e "  ${BOLD}A) Let azd provision Cosmos DB (Recommended):${NC}"
            echo "     • Uncomment Cosmos DB module in infra/main.bicep"
            echo "     • Run: azd up"
            echo "     • Bicep provisions Cosmos DB + grants MI access automatically"
            echo ""
            echo -e "  ${BOLD}B) Use existing Cosmos DB:${NC}"
            echo "     • Get Cosmos endpoint: az cosmosdb show --query documentEndpoint"
            echo "     • Get Container App MI: az containerapp show --query identity.principalId"
            echo "     • Grant role: az cosmosdb mongodb role assignment create"
            echo "     • Set COSMOS_ENDPOINT in Container App environment"
            echo ""
            print_info "Documentation:"
            echo "  • ${BOLD}docs/deployment/PRODUCTION_DATABASE_MANAGED_IDENTITY.md${NC}"
            echo "  • docs/deployment/COSMOS_DB_PORTAL_SETUP.md"
            echo "  • https://learn.microsoft.com/azure/cosmos-db/mongodb/how-to-setup-rbac"
            echo ""
            
            # For local .env, mark as not needed (production uses COSMOS_ENDPOINT)
            MONGODB_URI="<production-uses-COSMOS_ENDPOINT-not-MONGODB_URI>"
            MONGODB_DONE=true
            ;;
...
print_summary() {
    ...
    echo ""
    echo -e "  ${BOLD}1. Set up User-Assigned Managed Identity (UAMI) for azd:${NC}"
    ...
    echo ""
    echo -e "  ${BOLD}2. Set up Cosmos DB with Managed Identity (NOT connection strings!):${NC}"
    echo ""
    echo -e "     ${BOLD}Option A - Let azd provision Cosmos DB (Recommended):${NC}"
    echo "       • Edit infra/main.bicep and uncomment the Cosmos DB module"
    echo "       • Update Container App env to use COSMOS_ENDPOINT instead of MONGODB_URI"
    echo "       • Run: azd up"
    echo "       • Bicep automatically:"
    echo "         - Provisions Cosmos DB (serverless, MongoDB API)"
    echo "         - Creates System-Assigned MI for Container App"
    echo "         - Grants MI 'Cosmos DB Built-in Data Contributor' role"
    echo ""
    echo -e "     ${BOLD}Option B - Use existing Cosmos DB:${NC}"
    echo "       • Get Cosmos endpoint:"
    echo -e "         ${CYAN}az cosmosdb show --name NAME --query documentEndpoint${NC}"
    echo "       • After deploying Container App, get its MI Principal ID:"
    echo -e "         ${CYAN}az containerapp show --name NAME --query identity.principalId${NC}"
    echo "       • Grant MI access to Cosmos DB:"
    echo -e "         ${CYAN}az cosmosdb mongodb role assignment create \\${NC}"
    echo -e "         ${CYAN}  --role-definition-id 00000000-0000-0000-0000-000000000002 \\${NC}"
    echo -e "         ${CYAN}  --principal-id <MI_PRINCIPAL_ID>${NC}"
    echo "       • Set Container App env vars:"
    echo -e "         ${CYAN}COSMOS_ENDPOINT=https://YOUR_COSMOS.documents.azure.com${NC}"
    echo -e "         ${CYAN}COSMOS_DATABASE_NAME=template-doctor${NC}"
    echo ""
    echo -e "     ${BOLD}How Managed Identity works:${NC}"
    echo "       1. Container App acquires token from Azure AD automatically"
    echo "       2. Token is used as credentials in MongoDB connection (no secrets!)"
    echo "       3. Cosmos DB validates token against RBAC roles"
    echo "       4. Token auto-refreshes every 1 hour"
    echo ""
    print_info "     Full guide: ${BOLD}docs/deployment/PRODUCTION_DATABASE_MANAGED_IDENTITY.md${NC}"
    echo ""
