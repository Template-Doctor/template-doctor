...
export async function requireAuth(
    ...
    authLogger.info(
        { user: userInfo.login, path: req.path },
        "User authenticated",
    );
...
export function requireAdmin(req: Request, res: Response, next: NextFunction) {
    ...
    const adminUsers =
        process.env.ADMIN_GITHUB_USERS?.split(",").map((u) => u.trim()).filter(Boolean) || [];

    if (adminUsers.length === 0) {
        authLogger.warn(
            "No admin users configured. Set ADMIN_GITHUB_USERS environment variable.",
        );
    ...
    if (!adminUsers.includes(req.user.login)) {
        authLogger.warn(
            { user: req.user.login, path: req.path },
...
export async function requireSetupAccess(
    ...
    const allowedUsers =
        process.env.SETUP_ALLOWED_USERS?.split(",").map((u) => u.trim()).filter(Boolean) || [];

    if (allowedUsers.length === 0 || !allowedUsers.includes(userInfo.login)) {
        authLogger.warn(
            { user: userInfo.login, path: req.path },
            "Unauthorized setup access attempt",
        );
        return res.status(403).json({
            error: "Forbidden",
