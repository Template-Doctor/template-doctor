  ...
  function attachServerMethod(instance: AnalyzerLike){
    ...
    instance.analyzeTemplateServerSide = async function(repoUrl: string, ruleSetOrOptions?: any){
      try {
        ...
        const primaryEndpoint = (window as any).ApiRoutes && (window as any).ApiRoutes.build ? (window as any).ApiRoutes.build('analyze-template') : apiBase.replace(/\/$/, '') + '/api/v4/analyze-template';
        ...
        if(currentPort !== devPort && !primaryEndpoint.includes(':'+devPort)) {
          const alt = primaryEndpoint.replace(window.location.origin, window.location.protocol + '//' + window.location.hostname + ':' + devPort);
        ...
        for(const ep of candidates){
          try {
            ...
            if(!resp.ok){
              ...
              if((resp.status === 404 || resp.status === 502 || resp.status === 503) && ep !== candidates[candidates.length-1]) {
            ...
            if(ep !== primaryEndpoint) {
              console.info('[server-bridge] analyze-template fallback succeeded via', ep, 'after primary failure', lastErr);
            }
