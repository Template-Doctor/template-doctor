...
export default wrapHttp(async (req: any, _ctx: Context, requestId: string) => {
  ...
  if (req.method !== 'POST') return { status: 405, body: { error: 'Method Not Allowed', requestId } };

  const tokenHeader: string | undefined = req.headers['authorization'] || req.headers['Authorization'];
  const token = tokenHeader?.replace(/Bearer\s+/i, '').trim();
  if (!token) return { status: 401, body: { error: 'Authorization token is required', requestId } };

  const templateData: TemplateData | undefined = req.body;
  if (!validateTemplateData(templateData)) {
    return { status: 400, body: { error: 'Invalid or missing template data', requestId } };
  }
  ...
  try {
    ...
    try {
      ...
      const { data: defaultRef } = await octokit.git.getRef({ owner, repo, ref: `heads/${baseBranch}` });
    ...
    try {
      await octokit.repos.getContent({ owner, repo, path: folderPath, ref: branchName });
    } catch {
    ...
    try {
      const { data: latestFile }: any = await octokit.repos.getContent({ owner, repo, path: `${folderPath}/latest.json`, ref: sourceBranch });
    ...
    try {
      const { data: historyFile }: any = await octokit.repos.getContent({ owner, repo, path: `${folderPath}/history.json`, ref: sourceBranch });
    ...
    const { data: indexFile }: any = await octokit.repos.getContent({ owner, repo, path: 'packages/app/results/index-data.js', ref: branchName });
    const indexContent = Buffer.from(indexFile.content, 'base64').toString();
    const updatedIndex = addTemplateToIndexData(indexContent, templateData as any);
    await octokit.repos.createOrUpdateFileContents({
      owner,
      repo,
    ...
    return { status: 200, body: { success: true, prUrl: pr.html_url, requestId } };
  } catch (e: any) {
    return { status: 500, body: { error: 'Failed to create pull request', details: e?.message, requestId } };
  }
});

function validateTemplateData(d: any): d is TemplateData {
  return !!(d && typeof d.timestamp === 'number' && typeof d.repoUrl === 'string' && d.compliance && typeof d.compliance.percentage === 'number' && typeof d.compliance.issues === 'number' && typeof d.compliance.passed === 'number');
}
...
function getRepoIdentifier(url: string): string {
  try { const u = new URL(url); const p = u.pathname.split('/'); if (p.length >= 3) return `${p[1]}/${p[2]}`; return url.replace(/https?:\/\//, ''); } catch { return url; }
}

function getTemplateFolderName(url: string): string {
  try { const u = new URL(url); const p = u.pathname.split('/'); if (p.length >= 3) return `${p[1]}-${p[2]}`; } catch { /* noop */ }
...
function createResultData(templateData: TemplateData): string {
  ...
  const report = {
    ...
    compliance: {
      issues: Array.isArray(issues) ? issues : (typeof issues === 'number' ? Array(issues).fill(null).map((_, i) => ({ id: `issue-${i+1}`, severity: 'info', message: `Placeholder issue ${i+1}` })) : []),
      compliant: Array.isArray(passed) ? passed : (typeof passed === 'number' ? Array(passed).fill(null).map((_, i) => ({ id: `passed-${i+1}`, category: 'placeholder', message: `Placeholder passed check ${i+1}` })) : []),
      summary: `Issues found - Compliance: ${templateData.compliance.percentage}%`
    },
    upstreamTemplate: (typeof templateData.originUpstream === 'string' && templateData.originUpstream.includes('/')) ? templateData.originUpstream.trim() : undefined,
    history: [
      {
        timestamp: templateData.timestamp,
        ruleSet: templateData.ruleSet || 'default',
...
function createDashboardHtml(templateData: TemplateData, dataFileName: string): string {
...
function createLatestJson(templateData: TemplateData, dataFileName: string, dashboardFileName: string) {
  return {
    repoUrl: templateData.repoUrl,
    ruleSet: templateData.ruleSet || 'default',
    timestamp: templateData.timestamp,
...
function createHistoryEntry(templateData: TemplateData, dataFileName: string, dashboardFileName: string) {
  return {
    timestamp: templateData.timestamp,
    ruleSet: templateData.ruleSet || 'default',
    percentage: templateData.compliance.percentage,
...
function buildPrBody(t: TemplateData, folderPath: string, dashboardFile: string, dataFile: string): string {
