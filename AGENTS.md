...
⚠️ **Not Recommended**: This approach requires separate ports for frontend and backend, which breaks OAuth flow. Use Docker Compose instead.

If you must run services manually without Docker:
...
**Note**: OAuth will NOT work in this setup because frontend (4000) and backend (3001) are on different ports.

### Port Allocation

| Service                | Port | Notes                                    |
| ---------------------- | ---- | ---------------------------------------- |
| **Frontend + Backend** | 3000 | Combined Docker image (recommended)      |
| Express Backend        | 3001 | Manual development mode only             |
| Vite Dev Server        | 4000 | Manual development mode only             |
...
**Unit Tests** (`packages/server/tests/middleware/auth.test.ts`):
...
```bash
./scripts/smoke-api.sh            # assumes host at http://localhost:3000 and reads .env
BASE=http://localhost:3000 ./scripts/smoke-api.sh   # override base
...
**Note**: Default port is 3000 (combined Docker image). Use `BASE` to override for manual development mode.
...
- The frontend is fully TypeScript - all legacy JS files have been removed
- Express migration is in progress: check EXPRESS_MIGRATION_MATRIX.md for current status
...
`BASE` (in `.env`) is consumed by `scripts/smoke-api.sh` to know the Express server base URL for local probing (defaults `http://localhost:3000`).
`TD_BACKEND_BASE_URL` is exposed through the runtime-config endpoint to the browser for API calls when the frontend is pointed at a remote backend instance. During local dev with Docker they usually match, but they have distinct purposes—do not assume one automatically sets the other.
...
- [ ] Docs updated (this section and any relevant migration docs).
...
All legacy JavaScript files have been removed after confirming TypeScript parity and passing all tests. The following files were removed:
...
**Migration Complete**: All legacy JavaScript files have been removed. The TypeScript modules under `packages/app/src/` are now the sole authoritative source.
