...
async function getRunList(
    ...
    return fetchWithGitHubAuth(
        workflowUrl,
        { operationName: "getRunList" },
        context,
    );
}
...
async function triggerWorkflow(
    ...
    // Trigger workflow
    const rawResponse = await triggerAction(
        workflowOwner,
        workflowRepo,
        workflowId,
        workflowInput,
    ...
    if (context && context.log) {
        context.log(
            `Triggered workflow ${workflowOwner}/${workflowRepo}#${workflowId}`,
            {
                operation: "triggerWorkflow",
    ...
    const uniqueInputId =
        workflowInput &&
        Object.prototype.hasOwnProperty.call(workflowInput, runIdInputProperty)
    ...
    while (attempts < maxAttempts && !matchingRun) {
        ...
        if (context && context.log) {
            context.log(
                `Waiting ${waitTimeMs / 1000} seconds for GitHub to register the workflow run (attempt ${attempts + 1}/${maxAttempts})`,
                {
                    operation: "triggerWorkflow",
                    workflowOwner,
                    workflowRepo,
        ...
        if (context && context.log) {
            context.log(
                `Fetching recent runs to locate workflow run (attempt ${attempts + 1}/${maxAttempts})`,
                {
                    operation: "triggerWorkflow",
                    workflowOwner,
                    workflowRepo,
        ...
        const rawResponseFindRunId = await getRunList(
            workflowOwner,
            workflowRepo,
            workflowId,
            context,
        ...
        if (!rawResponseFindRunId.ok) {
            ...
            if (context && context.log && context.log.error) {
                context.log.error(
                    `Failed to get workflow runs (attempt ${attempts + 1}/${maxAttempts})`,
                    {
                        operation: "triggerWorkflow",
                        workflowOwner,
                        workflowRepo,
        ...
        if (
            !rawDataFindRunId ||
            !Array.isArray(rawDataFindRunId.workflow_runs)
        ) {
            if (context && context.log && context.log.error) {
                context.log.error(
                    `Invalid response structure from GitHub (attempt ${attempts + 1}/${maxAttempts})`,
                    {
                        operation: "triggerWorkflow",
                        correlationId,
                        attemptNumber: attempts + 1,
        ...
        if (context && context.log) {
            context.log(
                `Received ${rawDataFindRunId.workflow_runs.length} workflow runs (attempt ${attempts + 1}/${maxAttempts})`,
                {
                    operation: "triggerWorkflow",
        ...
        matchingRun = rawDataFindRunId.workflow_runs.find((run) => {
            ...
            const commitMsg =
                run.head_commit && run.head_commit.message
                    ? String(run.head_commit.message)
                    : "";
            return (
                (title && title.includes(String(uniqueInputId))) ||
                (commitMsg && commitMsg.includes(String(uniqueInputId)))
            );
        });
        ...
        if (matchingRun) {
            if (context && context.log) {
                context.log(
                    `Matching run found on attempt ${attempts + 1}/${maxAttempts}`,
                    {
                        operation: "triggerWorkflow",
        ...
        if (context && context.log) {
            context.log(
                `No matching run found on attempt ${attempts + 1}/${maxAttempts}, will ${attempts < maxAttempts - 1 ? "retry" : "give up"}`,
                {
                    operation: "triggerWorkflow",
                    runIdInputProperty,
                    uniqueInputId,
...
module.exports = async function (context, req) {
    ...
    try {
        ...
        const [incomingWorkflowOwner, incomingWorkflowRepo] =
            workflowOrgRep.split("/");
        ...
        if (result.found) {
            ...
            context.res = {
                ...
                body: {
                    error:
                        result.error || "trigger-action: Could not locate run",
                    errorType,
                    data: null,
                    context: {
