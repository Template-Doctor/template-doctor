...
router.post('/batch-scan-start', async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post('/repo-fork', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    try {
      ...
      const isSamlError =
        err.status === 403 &&
        (err.message?.includes('SAML') || err.message?.includes('SSO'));
...
const PROVIDER =
  (process.env.ISSUE_AI_PROVIDER || '').toLowerCase() || (AZURE_OPENAI_ENDPOINT ? 'azure' : 'github');
