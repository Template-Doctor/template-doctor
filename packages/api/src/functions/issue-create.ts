 ...
 */
export async function issueCreateHandler(request: HttpRequest, context: InvocationContext) {
  const corsHeaders = {
  ...
  if (process.env.API_OFFLINE) {
    return {
      ...
      jsonBody: {
        ...
        childResults: (childIssues || []).map((c: any, i: number) => ({ title: c?.title, issueNumber: i + 2 }))
      }
    };
  }
