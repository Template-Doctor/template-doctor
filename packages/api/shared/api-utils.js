...
async function withRetry(asyncFn, options = {}) {
    ...
    while (attemptCount < maxAttempts) {
        ...
        if (context && context.log) {
            context.log(
                `Executing operation (attempt ${attemptCount}/${maxAttempts})`,
                {
                    operation,
        ...
        try {
            ...
            if (context && context.log) {
                context.log(
                    `Operation successful on attempt ${attemptCount}/${maxAttempts}`,
                    {
                        operation,
                        attempt: attemptCount,
                        maxAttempts,
            ...
            if (context && context.log) {
                const logFn =
                    attemptCount < maxAttempts
                        ...
                        : context.log.error || context.log;

                logFn(
                    `Operation failed (attempt ${attemptCount}/${maxAttempts})`,
                    {
                        operation,
                        attempt: attemptCount,
                        maxAttempts,
    ...
    // but as a safeguard:
    throw (
        lastError || new Error(`Failed operation after ${maxAttempts} attempts`)
    );
}
 ...
 */
function createFetchResponseRetryCheck(
...
async function withGitHubApiRetry(apiFn, params, options = {}) {
    ...
    return withRetry(() => apiFn(...params), defaultOptions);
}
