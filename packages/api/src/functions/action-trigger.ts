...
async function dispatchWorkflow(owner: string, repo: string, workflowId: string | number, inputs: Record<string, any>, token: string, ctx: Context) {
...
async function listRecentRuns(owner: string, repo: string, workflowId: string | number, token: string, sinceIso: string, ctx: Context) {
  const idPart = encodeURIComponent(String(workflowId));
  ...
  if (!res.ok) {
    ...
    ctx.log.warn('action-trigger: listRecentRuns failed', { status: res.status, text: text.slice(0,500) });
...
export default wrapHttp(async (req: any, ctx: Context, requestId: string) => {
  ...
  const body: TriggerBody = (req.body && typeof req.body === 'object') ? req.body : {};
  ...
  if (workflowId === undefined || workflowId === null || workflowId === '') {
    return { status: 400, body: { error: 'workflowId is required', errorType: 'MISSING_PARAMETER', requestId } };
  }
  if (!runIdInputProperty) {
    return { status: 400, body: { error: 'runIdInputProperty is required', errorType: 'MISSING_PARAMETER', requestId } };
  }
  const { owner, repo } = orgRep.value!;
  const uniqueInputId = Object.prototype.hasOwnProperty.call(workflowInput, runIdInputProperty) ? workflowInput[runIdInputProperty] : undefined;
  if (!uniqueInputId) {
    return { status: 400, body: { error: `Input property ${runIdInputProperty} is missing in workflowInput`, errorType: 'MISSING_INPUT_PROPERTY', requestId } };
  }
  // Ten minutes lookback for polling
  const tenMinutesAgoIso = new Date(Date.now() - 10 * 60 * 1000).toISOString();
  // Dispatch
  const dispatchRes = await dispatchWorkflow(owner, repo, workflowId, workflowInput, token, ctx);
  if (!dispatchRes.ok) {
    const text = await dispatchRes.text();
    return { status: 502, body: { error: `GitHub dispatch failed: ${dispatchRes.status} ${dispatchRes.statusText}`, details: text.slice(0,1000), errorType: 'GITHUB_API_ERROR', requestId } };
  }
  ...
  for (let attempt = 1; attempt <= maxAttempts && !foundRun; attempt++) {
    ...
    ctx.log('action-trigger: waiting for run registration', { attempt, waitSeconds: waitMs/1000, requestId });
    await new Promise(r => setTimeout(r, waitMs));
    const list = await listRecentRuns(owner, repo, workflowId, token, tenMinutesAgoIso, ctx);
    if (!list.ok) continue;
    const runs = list.json.workflow_runs || [];
    for (const r of runs) {
      const title = r.display_title || r.name || '';
      const msg = r.head_commit?.message || '';
      if ((title && title.includes(String(uniqueInputId))) || (msg && msg.includes(String(uniqueInputId)))) {
        foundRun = r; break;
      }
    }
    if (foundRun) {
      return {
        ...
        body: {
          ...
          context: { uniqueInputId, ownerRepo: `${owner}/${repo}/actions/workflows/${workflowId}/runs`, run: foundRun, requestId }
        }
      };
    }
  }
  return {
    ...
    body: {
      ...
      data: null,
      context: { uniqueInputId, ownerRepo: `${owner}/${repo}/actions/workflows/${workflowId}/runs`, attempts: 5, requestId }
    }
  };
});
