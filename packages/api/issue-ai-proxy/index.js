...
function rateLimitKey(req) {
    ...
    const repo =
        (req.headers &&
            (req.headers["x-repo"] || req.headers["x-template-repo"])) ||
        "";
    const ip =
        (req.headers &&
...
module.exports = async function (context, req) {
    ...
    if (PROVIDER === "azure") {
        ...
        if (!AZURE_OPENAI_API_KEY) {
            ...
            return sendError(
                context,
                400,
                "AZURE_OPENAI_API_KEY not set (or implement Managed Identity)",
            );
        }
    } else {
        // GitHub
        ...
        if (!token) {
            return sendError(
                context,
                400,
                "Missing GITHUB_MODELS_TOKEN (or GITHUB_TOKEN) for GitHub provider.",
            );
        }
    }
    ...
    if (rl.limited) {
        return sendError(
            ...
            429,
            "Rate limit exceeded",
            `Retry in ${Math.ceil(rl.retryIn / 1000)}s`,
        );
    }
    ...
    if (PROVIDER === "azure") {
        try {
            const azureResp = await withRetries(context, "azure", async () =>
                callAzureOpenAI({
            ...
            return sendError(
                context,
                502,
                "Azure model inference failed",
                e.message,
            );
        }
    } else {
        const token =
            process.env.GITHUB_MODELS_TOKEN || process.env.GITHUB_TOKEN; // re-evaluate after validation
        try {
            const completion = await withRetries(context, "github", async () =>
