...
describe('Analyzer category grouping', () => {
  it('groups issues and compliant items into categories', async () => {
    const mockFiles = [
      { path: 'README.md', type: 'file', content: Buffer.from('# Test\n## Features\n## Getting Started\n## Resources\n## Guidance\n').toString('base64') },
      { path: 'azure.yaml', type: 'file', content: Buffer.from('name: test\nservices:\n  web:\n    host: containerapp\n').toString('base64') },
      { path: 'LICENSE', type: 'file', content: Buffer.from('MIT License').toString('base64') },
      { path: 'SECURITY.md', type: 'file', content: Buffer.from('# Security').toString('base64') },
      { path: 'CONTRIBUTING.md', type: 'file', content: Buffer.from('# Contributing').toString('base64') },
      { path: 'CODE_OF_CONDUCT.md', type: 'file', content: Buffer.from('# Code of Conduct').toString('base64') },
      { path: 'infra/main.bicep', type: 'file', content: Buffer.from('resource test {}').toString('base64') },
      { path: '.github/workflows/azure-dev.yml', type: 'file', content: Buffer.from('name: Azure Dev').toString('base64') }
    ];
  ...
  it('correctly maps issue categories to standard category keys', async () => {
    const mockFiles = [
      { path: 'README.md', type: 'file', content: Buffer.from('# Test').toString('base64') },
      { path: 'infra/main.bicep', type: 'file', content: Buffer.from('resource test {}').toString('base64') }
    ];
    ...
    const missingFileIssues = repoMgmt.issues.filter(i => 
  ...
  it('calculates correct percentage for each category', async () => {
    const mockFiles = [
      { path: 'README.md', type: 'file', content: Buffer.from('# Test\n## Features\n## Getting Started\n## Resources\n## Guidance\n').toString('base64') },
      { path: 'azure.yaml', type: 'file', content: Buffer.from('name: test\nservices:\n  web:\n    host: containerapp\n').toString('base64') },
      { path: 'LICENSE', type: 'file', content: Buffer.from('MIT').toString('base64') },
      { path: 'infra/main.bicep', type: 'file', content: Buffer.from('resource test {}').toString('base64') }
    ];
  ...
  it('groups bicep security issues into security category', async () => {
    const mockFiles = [
      ...
      { path: 'azure.yaml', type: 'file', content: Buffer.from('name: test\nservices:\n  web:\n    host: containerapp\n').toString('base64') },
      { path: 'LICENSE', type: 'file', content: Buffer.from('MIT').toString('base64') },
      { path: 'infra/main.bicep', type: 'file', content: Buffer.from(`
resource storage 'Microsoft.Storage/storageAccounts@2021-06-01' = {
  ...
  it('groups deployment-related items into deployment category', async () => {
    ...
    const deploymentRelated = allDeploymentItems.some(i => 
