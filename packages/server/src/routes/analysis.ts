 ...
 */
router.post('/submit-analysis-dispatch', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const fromPayload = 
      (typeof client_payload.targetRepo === 'string' && client_payload.targetRepo) ||
      (typeof client_payload.repoSlug === 'string' && client_payload.repoSlug) ||
      '';
    ...
    if (!response.ok) {
      ...
      console.error('[submit-analysis-dispatch] GitHub dispatch failed', {
        requestId,
...
router.post('/archive-collection', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    console.error('[archive-collection] failure', { error: error?.message });
    next(error);
  }
});
  ...
  const report = {
    ...
    compliance: {
      issues: Array.isArray(issues) ? issues : 
        typeof issues === 'number' ? Array(issues).fill(null).map((_, i) => ({
          id: `issue-${i + 1}`,
          severity: 'info',
          message: `Placeholder issue ${i + 1}`
        })) : [],
      compliant: Array.isArray(passed) ? passed :
        typeof passed === 'number' ? Array(passed).fill(null).map((_, i) => ({
          id: `passed-${i + 1}`,
    ...
    upstreamTemplate: typeof templateData.originUpstream === 'string' && 
      templateData.originUpstream.includes('/') ? templateData.originUpstream.trim() : undefined,
    history: [{
      timestamp: templateData.timestamp,
      ruleSet: templateData.ruleSet || 'default',
