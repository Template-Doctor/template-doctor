...
module.exports = async function (context, req) {
    ...
    try {
        ...
        if (!dispatchRes.ok) {
            ...
            throw new Error(
                `GitHub dispatch failed: ${dispatchRes.status} ${dispatchRes.statusText} - ${errText}`,
            );
        }
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                context.log.warn(
                    `Run discovery attempt ${attempt} failed: ${e.message}`,
                );
            }
        ...
        context.res = {
            ...
            body: {
                ...
                message: githubRunId
                    ...
                    : "Workflow triggered; run discovery in progress",
            },
        };
    } catch (err) {
        context.log.error("validate-template error:", err);
        const isGitHubError =
            err.message && err.message.includes("GitHub dispatch failed");
        context.res = {
