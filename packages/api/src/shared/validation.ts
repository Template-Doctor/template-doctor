...
export function parseOwnerRepo(value: string | undefined, requestId: string) {
  if (!value) {
    return { error: { status: 400, body: { error: 'workflowOrgRep is required', errorType: 'MISSING_PARAMETER', requestId } } };
  }
  const parts = value.split('/');
  if (parts.length !== 2 || !parts[0] || !parts[1]) {
    return { error: { status: 400, body: { error: 'workflowOrgRep must be in owner/repo format', errorType: 'INVALID_FORMAT', requestId } } };
  }
...
export function requireRunId(runId: string | number | undefined, requestId: string) {
  if (!runId) {
    return { error: { status: 400, body: { error: 'workflowRunId is required', errorType: 'MISSING_PARAMETER', requestId } } };
  }
  ...
  if (!Number.isFinite(numeric)) {
    return { error: { status: 400, body: { error: 'workflowRunId must be numeric', errorType: 'INVALID_FORMAT', requestId } } };
  }
...
export function isPost(method: string, requestId: string) {
  if (method !== 'POST') {
    return { error: { status: 405, body: { error: 'Method Not Allowed', requestId } } };
  }
  return {};
}
