...
module.exports = async function (context, req) {
    ...
    try {
        ...
        try {
            ...
            context.log(
                `Found base branch: ${baseBranch}, SHA: ${baseCommitSha}`,
            );
        } catch (branchError) {
            // Try default branch if specified branch doesn't exist
            context.log.error(
                `Branch ${baseBranch} not found: ${branchError.message}`,
            );
            ...
            context.log(
                `Using default branch: ${defaultBranch}, SHA: ${baseCommitSha}`,
            );
        }
        ...
        try {
            ...
            latestSha =
                latestFile && latestFile.sha ? latestFile.sha : undefined;
        } catch (e) {
            context.log(
                `latest.json not found for ${folderPath}, will create new one.`,
            );
        }
        const latestJsonObj = createLatestJson(
            templateData,
            dataFileName,
            dashboardFileName,
        );
        ...
        try {
            ...
            context.log(
                `history.json not found for ${folderPath}, will create new one.`,
            );
        }
        historyArray.push(
...
function createResultData(templateData) {
    ...
    const issues = templateData.compliance.issues
        ? Array.isArray(templateData.compliance.issues)
            ...
            : Array(templateData.compliance.issues)
                  .fill()
                  .map((_, i) => ({
                      id: `issue-${i + 1}`,
                      severity: "info",
    ...
    const compliant = templateData.compliance.passed
        ? Array(templateData.compliance.passed)
              .fill()
              .map((_, i) => ({
                  id: `passed-${i + 1}`,
    ...
    try {
        ...
        if (fs.existsSync(templatePath)) {
            ...
            htmlTemplate = htmlTemplate
                ...
                .replace(
                    /\{\{PASSED_COUNT\}\}/g,
                    templateData.compliance.passed,
                )
                .replace(
                    /\{\{TOTAL_CHECKS\}\}/g,
                    templateData.compliance.issues +
