    ...
    result.categories.forEach(category => {
      if (category.checks && Array.isArray(category.checks)) {
        category.checks.forEach(check => {
          ...
          if (check.status === 'passed') compliant.push(item); else issues.push(item);
        });
      }
    });
  }
...
if(!(window as any).__TD_adaptResultData){ (window as any).__TD_adaptResultData = adaptResultData; }
