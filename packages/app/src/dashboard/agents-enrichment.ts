...
function debug(msg:string, data?:any){ try { console.debug('[AgentsEnrichment]', msg, data||''); } catch(_){} }
...
function applyAgentsCachedResult(adaptedData:AdaptedDataLike, cached:any){
  ...
  if (cached.status === 'missing') {
    ...
    adaptedData.compliance.compliant.push({ id: 'agents-doc-valid', category: 'agents', message: `agents.md valid (${cached.agentCount || 0} ${agentLabel})` });
  }
}
...
function updateAgentsBadge(issue:any, compliant:any){
  try {
    ...
    if (issue && issue.id === 'agents-missing-file') {
      badge.textContent = 'Agents: Missing'; badge.style.background = '#d9534f'; badge.style.color = '#fff'; badge.title = 'agents.md not found in repository'; updateAgentsTileStatus('missing');
    } else if (issue) {
      badge.textContent = 'Agents: Invalid'; badge.style.background = '#ff9800'; badge.style.color = '#fff'; badge.title = 'agents.md formatting problems'; updateAgentsTileStatus('invalid');
    } else if (compliant) {
      badge.textContent = 'Agents: OK'; badge.style.background = '#28a745'; badge.style.color = '#fff'; badge.title = 'agents.md validated'; updateAgentsTileStatus('valid');
    }
  } catch(_) {}
}
...
function updateAgentsTileStatus(status:string){
  try {
    ...
    if (status === 'missing') {
      tile.style.background = '#ffe5e5'; tile.style.border = '1px solid #d9534f';
      if (!tile.querySelector('.agents-action')) {
        ...
        btn.onclick = (e)=>{ e.preventDefault(); (window as any).createAgentsMdIssue && (window as any).createAgentsMdIssue(); };
        tile.appendChild(btn);
      }
    } else if (status === 'invalid') {
      tile.style.background = '#fff3e0'; tile.style.border = '1px solid #ff9800';
      const existing = tile.querySelector('.agents-action'); if(existing) existing.remove();
    } else if (status === 'valid') {
      tile.style.background = '#e6f7ed'; tile.style.border = '1px solid #28a745';
      const existing = tile.querySelector('.agents-action'); if(existing) existing.remove();
    }
  } catch(_) {}
}
...
export async function runAgentsEnrichment(adaptedData:AdaptedDataLike){
  ...
  if (existingItems.length) { debug('Skip enrichment (already present)'); updateAgentsBadgeFromData(adaptedData); return; }
  const repoUrl = adaptedData.repoUrl || '';
  if (!/https?:\/\/github\.com\//i.test(repoUrl)) { debug('Skip (non-GitHub repo)'); return; }
  const ownerRepoMatch = repoUrl.match(/github\.com\/([^/]+)\/([^/]+)(?:\.git)?/i); if(!ownerRepoMatch) return;
  const owner = ownerRepoMatch[1]; const repo = ownerRepoMatch[2];
  const cacheKey = `__TD_agents_cache_${owner}_${repo}`;
  try { const cached = sessionStorage && sessionStorage.getItem(cacheKey); if(cached){ const parsed = JSON.parse(cached); applyAgentsCachedResult(adaptedData, parsed); updateAgentsBadgeFromData(adaptedData); return; } } catch(_){}
  ...
  for (const branch of candidateBranches){
    try {
      ...
      if (cdnResp.ok){ content = await cdnResp.text(); break; }
      const rawResp = await fetch(`https://raw.githubusercontent.com/${owner}/${repo}/${branch}/agents.md`, { cache:'no-store' });
      if (rawResp.ok){ content = await rawResp.text(); break; }
    } catch(_){}
  }
  if (content == null){
    const issue = { id:'agents-missing-file', category:'agents', severity:'error', message:'agents.md file is missing (client check)', recommendation:'Add an agents.md defining agents (name, description, inputs, outputs, permissions).' };
    adaptedData.compliance.issues.push(issue);
    if (!adaptedData.compliance.categories) adaptedData.compliance.categories = {};
    if (!adaptedData.compliance.categories.agents){ adaptedData.compliance.categories.agents = { enabled:true, issues:[issue], compliant:[], percentage:0 }; }
    storeAgentsCache(cacheKey, { status:'missing', problems:['file not found'] });
    updateAgentsBadge(issue, null); updateAgentsTileStatus('missing');
    return;
  }
  ...
  let headerCols:string[] = []; if(tableHeaderLine){ headerCols = tableHeaderLine.split('|').map(c=>c.trim().toLowerCase()).filter(Boolean); }
  ...
  if(hasTable){
    ...
    for(let i=tableIndex+1;i<lines.length;i++){
      ...
      if(!/\|/.test(ln)){ if(ln.trim()==='') break; continue; }
      const cellParts = ln.split('|').map(c=>c.trim());
      if(cellParts.filter(Boolean).length >= 2){ agentCount++; }
    }
  }
  if(problems.length){
    const issue = { id:'agents-format-invalid', category:'agents', severity:'warning', message:'agents.md present but formatting issues detected (client check)', details:problems };
    adaptedData.compliance.issues.push(issue);
    storeAgentsCache(cacheKey, { status:'invalid', problems, agentCount });
    updateAgentsBadge(issue, null); updateAgentsTileStatus('invalid');
  } else {
    const compliantItem = { id:'agents-doc-valid', category:'agents', message:`agents.md present and basic structure validated (${agentCount} agent${agentCount === 1 ? '' : 's'})`, details:{ agentCount, columns: headerCols } };
    adaptedData.compliance.compliant.push(compliantItem);
    storeAgentsCache(cacheKey, { status:'valid', agentCount });
    updateAgentsBadge(null, compliantItem); updateAgentsTileStatus('valid');
  }
}

// Export helpers if legacy code still calls them directly
if(!(window as any).__TD_agents){
  (window as any).__TD_agents = { runAgentsEnrichment, updateAgentsBadge, updateAgentsBadgeFromData, updateAgentsTileStatus };
}
