  ...
  try {
    ...
    if (gh && typeof gh.createIssueGraphQL === 'function') {
      try {
        ...
        const gqlIssue = await gh.createIssueGraphQL(
          owner,
          repo,
          title,
          body,
    ...
    if (!created) {
      ...
      try {
        ...
        console.warn('[AgentsEnrichment] REST with assignee failed, retrying simplified:', assignErr.message);
    ...
    if (created) {
      ...
      const tileBtn = document.querySelector('.category-breakdown .tile[data-category="agents"] .agents-action button') as HTMLButtonElement;
      if (tileBtn) {
