...
export default wrapHttp(async (req: any, _ctx: Context, requestId: string) => {
  ...
  const baseUrl = getMergedValue('TD_BACKEND_BASE_URL', process.env.TD_BACKEND_BASE_URL || process.env.BACKEND_BASE_URL || '');
  ...
  const defaultRuleSet = getMergedValue('DEFAULT_RULE_SET', process.env.DEFAULT_RULE_SET || process.env.TD_DEFAULT_RULE_SET || '');
  const requireAuthForResults = getMergedValue('REQUIRE_AUTH_FOR_RESULTS', process.env.REQUIRE_AUTH_FOR_RESULTS || process.env.TD_REQUIRE_AUTH_FOR_RESULTS || '');
  const autoSaveResults = getMergedValue('AUTO_SAVE_RESULTS', process.env.AUTO_SAVE_RESULTS || process.env.TD_AUTO_SAVE_RESULTS || '');
  const archiveEnabled = getMergedValue('ARCHIVE_ENABLED', process.env.TD_ARCHIVE_ENABLED || process.env.ARCHIVE_ENABLED || '');
  const archiveCollection = getMergedValue('ARCHIVE_COLLECTION', process.env.TD_ARCHIVE_COLLECTION || process.env.ARCHIVE_COLLECTION || '');
  const dispatchTargetRepo = getMergedValue('DISPATCH_TARGET_REPO', process.env.DISPATCH_TARGET_REPO || process.env.TD_DISPATCH_TARGET_REPO || '');
  const issueAIEnabled = getMergedValue('ISSUE_AI_ENABLED', process.env.ISSUE_AI_ENABLED || process.env.TD_ISSUE_AI_ENABLED || '');
