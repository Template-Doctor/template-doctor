...
async function githubCancelRun({ owner, repo, runId, token }) {
    ...
    throw new Error(
        `GitHub cancel failed: ${res.status} ${res.statusText} - ${body}`,
    );
}
...
module.exports = async function (context, req) {
    ...
    context.log("API - validation-cancel called", {
        repo: `${owner}/${repo}`,
        repoSource,
        isProd: !!websiteInstanceId,
    });
    ...
    try {
        ...
        const b = req.body || {};
        const localRunId =
            q.runId || b.runId || q.localRunId || b.localRunId || null; // optional, for logging
        ...
        if (!githubRunId) {
            if (!localRunId) {
                context.log.warn(
                    "validation-cancel missing both githubRunId and local runId; cannot discover.",
                );
                context.res = {
                    status: 400,
                    headers: corsHeaders(),
            ...
            context.log(
                `validation-cancel: attempting discovery for runId ${localRunId} in ${owner}/${repo} using workflow '${workflowFile}' on branch '${branch}'`,
            );
            ...
            try {
                ...
                for (const r of candidates) {
                    ...
                    const commitMsg =
                        r.head_commit && r.head_commit.message
                            ...
                            : "";
                    if (
                        (title && String(title).includes(localRunId)) ||
                        commitMsg.includes(localRunId)
                    ) {
                        discovered = r;
                        break;
                    }
                ...
                context.log.warn(
                    `validation-cancel: workflow runs list failed: ${wfErr.message}`,
                );
            }
            if (!discovered) {
                try {
                    ...
                    context.log.warn(
                        `validation-cancel: repo runs list failed: ${repoErr.message}`,
                    );
                }
            }
            ...
            if (discovered && discovered.id) {
                ...
                context.log(
                    `validation-cancel: discovered githubRunId ${githubRunId} for local runId ${localRunId}`,
                );
            } else {
                context.res = {
                    status: 400,
                    ...
                    body: {
                        ...
                        debug: isLocal
                            ? { owner, repo, branch, workflowFile, localRunId }
                            : undefined,
                    },
                };
                return;
        ...
        context.log(
            `Cancelling GitHub workflow run ${githubRunId} in ${owner}/${repo}...`,
            { localRunId },
        );
        ...
        context.res = {
            ...
            body: {
                ...
                suggestion:
                    status === 401
                        ? "Verify GH_WORKFLOW_TOKEN, scopes (workflow), and SSO organization authorization."
                        : undefined,
            },
        };
    }
