...
async function getOSSFScore(
    ...
    context.log(
        `Getting OSSF score using action APIs for ${templateOwnerRepo} with minimum score: ${minScore.toFixed(1)}`,
    );

    // Validate input parameters
    if (
        !templateOwnerRepo ||
        typeof templateOwnerRepo !== "string" ||
        templateOwnerRepo.indexOf("/") === -1
    ) {
        addIssue(
            issues,
            "ossf-score-invalid-template-repo",
            "warning",
            "Invalid template repo string for OSSF score. Use owner/repo format.",
        );
    ...
    if (!requestGuid || typeof requestGuid !== "string") {
        addIssue(
            issues,
            "ossf-score-invalid-request-guid",
            "warning",
            "Invalid request GUID for OSSF score.",
        );
        return { score: null, runId: null };
    }
    ...
    try {
        ...
        if (triggerResult.status !== 200 || !triggerResult.found) {
            addIssue(
                issues,
                "ossf-score-workflow-trigger-failed",
                "warning",
        ...
        context.log(
            `OSSF score workflow triggered successfully with run ID: ${runId}`,
        );
        ...
        while (attempts < maxAttempts) {
            ...
            if (!run || typeof run.status === "undefined") {
                context.log.warn(
                    `OSSF workflow check received invalid response, attempt: ${attempts + 1}/${maxAttempts}`,
                );
                attempts++;
                continue;
            }
            ...
            context.log(
                `Checking OSSF workflow status: ${run.status}, attempt: ${attempts + 1}/${maxAttempts}`,
            );
        ...
        const runStatus = artifacts.artifacts.find(
            (artifact) =>
                typeof artifact.name === "string" &&
                artifact.name.includes(requestGuid),
        );

        if (!runStatus) {
            addIssue(
                issues,
                "ossf-score-artifact-failed",
                "warning",
        ...
        if (
            !runStatus.archive_download_url ||
            runStatus.archive_download_url.length < 5
        ) {
            addIssue(
                issues,
                "ossf-score-artifact-download-failed",
                "warning",
                `OSSF workflow concluded without finding artifact download URL`,
                runStatus,
            );
            return { score: null, runId };
        }
        ...
        if (!scoreRaw) {
            addIssue(
                issues,
                "ossf-score-value-not-found",
                "warning",
                `OSSF workflow concluded without finding score value: ${runStatus.url}`,
                runStatus,
            );
            return { score: null, runId };
        }
        ...
        if (isNaN(score)) {
            addIssue(
                issues,
                "ossf-score-invalid-number",
                "warning",
                `OSSF workflow returned invalid score value: ${scoreString}`,
                runStatus,
            );
            return { score: null, runId };
        }
        ...
        if (Math.abs(score - minScore) < epsilon || score > minScore) {
            compliance.push({
                ...
                details: {
                    templateOwnerRepo: templateOwnerRepo,
        ...
        addIssue(
            ...
            "ossf-score-error",
            "warning",
            "Failed to fetch OSSF Scorecard",
