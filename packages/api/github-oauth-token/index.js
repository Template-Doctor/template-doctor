...
module.exports = async function (context, req) {
    ...
    const allowedOrigins = (
        process.env.GITHUB_OAUTH_ALLOWED_ORIGINS || "http://localhost:8080"
    )
        .split(",")
        .map((o) => o.trim())
        .filter(Boolean);
    ...
    const corsOrigin =
        allowAll || allowedOrigins.includes(origin)
    ...
    context.log("GitHub OAuth token exchange invoked", {
        requestId,
        origin,
    ...
    const expectedState =
        process.env.GITHUB_OAUTH_EXPECT_STATE === "1"
            ? body.expectedState || null
            : null; // optional future hook
    ...
    if (!client_id || !client_secret) {
        ...
        context.res.body = {
            ...
            requestId,
            missing: { client_id: !client_id, client_secret: !client_secret },
        };
        return;
    }
    ...
    try {
        ...
        context.log("GitHub OAuth response", {
            requestId,
            status: ghRes.status,
            hasError: !!data.error,
        });
        ...
        context.log.error("GitHub OAuth exchange exception", {
            requestId,
            error: err.message,
        });
        context.res.status = 500;
