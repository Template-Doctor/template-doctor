...
test.describe('Category breakdown tiles display', () => {
  ...
  test('renders all six category tiles with correct structure', async ({ page }) => {
    ...
    const mockResult = {
      ...
      compliance: {
        issues: [
          { id: 'missing-file-license', category: 'file', message: 'Missing LICENSE' }
        ],
        compliant: [
  ...
  test('shows disabled badge in gray for disabled categories', async ({ page }) => {
    ...
    const disabledBadges = page.locator('.category-breakdown .badge').filter({ hasText: 'Disabled' });
    const count = await disabledBadges.count();
    expect(count).toBe(2);

    // Check disabled badge styling
    const firstDisabledBadge = disabledBadges.first();
    const bgColor = await firstDisabledBadge.evaluate(el => window.getComputedStyle(el).backgroundColor);
    // #6c757d is gray
