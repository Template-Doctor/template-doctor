 ...
 */
export async function repoForkHandler(request: HttpRequest, context: InvocationContext) {
  const cors = {
  ...
  let body: any;
  try { body = await readJson(request); } catch { body = undefined; }
  ...
  const forkOwner = targetOwner || authedUser || process.env.FORK_FALLBACK_OWNER || 'fallback-user';
  ...
  try {
    ...
    return { status: 502, headers: cors, jsonBody: { error: 'Failed to initiate fork', details: classification.message, documentationUrl: classification.documentationUrl } };
  }
