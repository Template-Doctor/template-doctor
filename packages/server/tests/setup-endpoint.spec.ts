...
describe('Setup Endpoint - Git CSV Persistence', () => {
  ...
  it('should save and load configuration overrides (POST + GET)', async () => {
    ...
    try {
      // Save overrides
      const postResponse = await fetch('http://localhost:3001/api/v4/setup', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          user: 'test-user',
      ...
      const csvExists = await fs.access(CONFIG_FILE).then(() => true).catch(() => false);
      expect(csvExists).toBe(true);

      // Load overrides
      const getResponse = await fetch('http://localhost:3001/api/v4/setup');
  ...
  it('should delete overrides when value is null (POST)', async () => {
    ...
    try {
      ...
      await fetch('http://localhost:3001/api/v4/setup', {
        ...
        body: JSON.stringify({
          user: 'admin',
          overrides: { key1: 'value1', key2: 'value2', key3: 'value3' },
        }),
      });
