...
function runAzdProvisionTest() {
  ...
  const validateUrl = window.ApiRoutes ? window.ApiRoutes.build('validation-template') : joinUrl(apiBase, '/api/v4/validation-template');
  ...
  const statusUrlBase = window.ApiRoutes ? window.ApiRoutes.build('validation-status') : joinUrl(apiBase, '/api/v4/validation-status');
      ...
      if (stopBtn) {
        ...
        stopBtn.onclick = async () => {
          try {
            ...
            const cancelUrl = window.ApiRoutes ? window.ApiRoutes.build('validation-cancel') : joinUrl(apiBase, '/api/v4/validation-cancel');
