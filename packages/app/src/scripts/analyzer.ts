...
class TemplateAnalyzer {
  ...
  async analyzeTemplateServerSide(repoUrl, ruleSetOrOptions) { this.debug('analyzer', `Using server-side analysis for ${repoUrl} with ruleset: ${ruleSetOrOptions}`); try { // Backward/forgiving signature: second param may be ruleSet string or options object from tests
...
if (!existingTA || typeof existingTA !== 'object' || !('analyzeTemplateServerSide' in existingTA)) {
  ...
  (existingTA as any).analyzeTemplateServerSide = function(repoUrl, ruleSet){
