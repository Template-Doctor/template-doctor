(() => {
  // packages/app/src/scripts/issue-format.ts
  function mapAnalyzerIssueToViolation(issue) {
    ...
    return {
      ...
      path: issue.path,
      filePath: issue.path,
      // preserve for legacy consumers that expect filePath
  ...
  function formatViolationAsIssue(v, options) {
    const labels = [];
    ...
    if (effectivePath) labels.push("path");
    const bodyLines = [];
...
Compliance Snapshot: ${options.compliancePercentage}%`);
  ...
  var TemplateAnalyzerDocs = class {
    async getConfig() {
      ...
      if (cfg.requiredWorkflowFiles) {
        cfg.requiredWorkflowFiles = cfg.requiredWorkflowFiles.map((item) => ({
    ...
    evaluateDefaultBranchRule(config, _repoInfo, defaultBranch, issues, compliant) {
  ...
  var TemplateAnalyzer = class {
    ...
    async loadRuleSetConfigs() {
      try {
        ...
        if (this.ruleSetConfigs.dod.requiredWorkflowFiles) {
          this.ruleSetConfigs.dod.requiredWorkflowFiles = this.ruleSetConfigs.dod.requiredWorkflowFiles.map((item) => ({ pattern: new RegExp(item.pattern, "i"), message: item.message }));
        }
        const partnerResponse = await fetch("./configs/partner-config.json");
        if (!partnerResponse.ok) throw new Error(`Failed to load Partner config: ${partnerResponse.status}`);
        this.ruleSetConfigs.partner = await partnerResponse.json();
        if (this.ruleSetConfigs.partner.requiredWorkflowFiles) {
          this.ruleSetConfigs.partner.requiredWorkflowFiles = this.ruleSetConfigs.partner.requiredWorkflowFiles.map((item) => ({ pattern: new RegExp(item.pattern, "i"), message: item.message }));
        }
        ...
        if (!customResponse.ok) throw new Error(`Failed to load Custom config: ${customResponse.status}`);
        this.ruleSetConfigs.custom = await customResponse.json();
        if (this.ruleSetConfigs.custom.requiredWorkflowFiles) {
          this.ruleSetConfigs.custom.requiredWorkflowFiles = this.ruleSetConfigs.custom.requiredWorkflowFiles.map((item) => ({ pattern: new RegExp(item.pattern, "i"), message: item.message }));
        }
        console.log("Rule set configurations loaded");
      } catch (error) {
        console.error("Failed to load rule set configurations:", error);
        this.ruleSetConfigs.dod = { requiredFiles: ["README.md", "azure.yaml", "LICENSE"], requiredFolders: ["infra", ".github"], requiredWorkflowFiles: [{ pattern: /\.github\/workflows\/azure-dev\.yml/i, message: "Missing required GitHub workflow: azure-dev.yml" }], readmeRequirements: { requiredHeadings: ["Prerequisites", "Getting Started"], architectureDiagram: { heading: "Architecture", requiresImage: true } } };
        this.ruleSetConfigs.partner = { ...this.ruleSetConfigs.dod, requiredFiles: ["README.md", "azure.yaml"] };
        this.ruleSetConfigs.custom = { requiredFiles: ["README.md", "azure.yaml"], requiredFolders: ["infra"] };
      }
    }
    ...
    checkReadmeRequirements(readmeContent, issues, compliant, config) {
      ...
      if (config.readmeRequirements?.requiredHeadings) {
        for (const requiredHeading of config.readmeRequirements.requiredHeadings) {
          ...
          if (!headingMatch) {
            issues.push({ id: `readme-missing-heading-${requiredHeading.toLowerCase().replace(/\s+/g, "-")}`, severity: "error", message: `README.md is missing required h2 heading: ${requiredHeading}`, error: `README.md does not contain required h2 heading: ${requiredHeading}`, filePath: "README.md", snippet: readmeSnippet });
          } else {
            compliant.push({ id: `readme-heading-${requiredHeading.toLowerCase().replace(/\s+/g, "-")}`, category: "readmeHeading", message: `README.md contains required h2 heading: ${requiredHeading}`, details: { heading: requiredHeading, level: headingMatch.level } });
          }
        }
      }
      if (config.readmeRequirements?.architectureDiagram) {
        ...
        const architectureHeading = headings.find((h) => h.level === 2 && h.text.toLowerCase() === heading.toLowerCase());
        if (!architectureHeading) {
          issues.push({ id: "readme-missing-architecture-diagram-heading", severity: "error", message: `README.md is missing required h2 heading: ${heading}`, error: `README.md does not contain required h2 heading: ${heading}`, filePath: "README.md", snippet: readmeSnippet });
        } else {
          compliant.push({ id: "readme-architecture-diagram-heading", category: "readmeHeading", message: `README.md contains required h2 heading: ${heading}`, details: { heading, level: architectureHeading.level } });
          if (requiresImage && !architectureHeading.hasImage) {
            issues.push({ id: "readme-missing-architecture-diagram-image", severity: "error", message: `Architecture Diagram section does not contain an image`, error: `README.md has Architecture Diagram heading but is missing an image`, filePath: "README.md", snippet: readmeSnippet });
          } else if (requiresImage && architectureHeading.hasImage) {
            compliant.push({ id: "readme-architecture-diagram-image", category: "readmeImage", message: `Architecture Diagram section contains an image`, details: { heading } });
          }
        }
      }
    }
    async analyzeTemplate(repoUrl, ruleSet = "dod") {
      if (!ruleSet || ruleSet === "dod") {
        const cfg2 = window.TemplateDoctorConfig || {};
        if (cfg2.defaultRuleSet && typeof cfg2.defaultRuleSet === "string") {
          ruleSet = cfg2.defaultRuleSet;
        }
      }
      const cfg = window.TemplateDoctorConfig || {};
      const preferServerSide = cfg.preferServerSideAnalysis === true || cfg.analysis?.useServerSide === true;
      const allowFallback = false;
      if (preferServerSide) {
        try {
          const result = await this.analyzeTemplateServerSide(repoUrl, ruleSet);
          window.TemplateDoctorRuntime = Object.assign({}, window.TemplateDoctorRuntime, { lastMode: "server", lastServerAttemptFailed: false, fallbackUsed: false });
          document.dispatchEvent(new CustomEvent("analysis-mode-changed"));
          return result;
        } catch (err) {
          console.error("[analyzer] Server-side analysis failed", err);
          window.TemplateDoctorRuntime = Object.assign({}, window.TemplateDoctorRuntime, { lastMode: "server-failed", lastServerAttemptFailed: true });
          document.dispatchEvent(new CustomEvent("analysis-mode-changed"));
          throw err instanceof Error ? err : new Error(String(err));
        }
      }
      window.TemplateDoctorRuntime = Object.assign({}, window.TemplateDoctorRuntime, { lastMode: "client", fallbackUsed: false });
      document.dispatchEvent(new CustomEvent("analysis-mode-changed"));
      ...
      try {
        ...
        for (const file of config.requiredFiles) {
          if (!normalized.includes(file.toLowerCase())) {
            issues.push({ id: `missing-${file}`, severity: "error", message: `Missing required file: ${file}`, error: `File ${file} not found in repository` });
          } else {
            compliant.push({ id: `file-${file}`, category: "requiredFile", message: `Required file found: ${file}`, details: { fileName: file } });
          }
        }
        if (config.requiredWorkflowFiles) {
          for (const workflowFile of config.requiredWorkflowFiles) {
            ...
            if (!matchingFile) {
              issues.push({ id: `missing-workflow-${workflowFile.pattern.source}`, severity: "error", message: workflowFile.message, error: workflowFile.message });
            } else {
              compliant.push({ id: `workflow-${matchingFile}`, category: "requiredWorkflow", message: `Required workflow file found: ${matchingFile}`, details: { fileName: matchingFile, patternMatched: workflowFile.pattern.source } });
            }
          }
        }
        for (const folder of config.requiredFolders) {
          if (!normalized.some((f) => f.startsWith(folder.toLowerCase() + "/"))) {
            issues.push({ id: `missing-folder-${folder}`, severity: "error", message: `Missing required folder: ${folder}/`, error: `Folder ${folder} not found in repository` });
          } else {
            const folderFiles = normalized.filter((f) => f.startsWith(folder.toLowerCase() + "/"));
            compliant.push({ id: `folder-${folder}`, category: "requiredFolder", message: `Required folder found: ${folder}/`, details: { folderPath: folder, fileCount: folderFiles.length } });
          }
        }
        if (config.readmeRequirements && normalized.some((f) => f === "readme.md")) {
          try {
            ...
            issues.push({ id: "readme-read-error", severity: "warning", message: "Could not read README.md", error: err instanceof Error ? err.message : String(err) });
          }
        }
        const bicepFiles = files.filter((f) => f.startsWith("infra/") && f.endsWith(".bicep"));
        if (bicepFiles.length === 0) {
          issues.push({ id: "missing-bicep", severity: "error", message: "No Bicep files found in infra/", error: "No Bicep files found in the infra/ directory" });
        } else {
          compliant.push({ id: "bicep-files-exist", category: "bicepFiles", message: `Bicep files found in infra/ directory: ${bicepFiles.length} files`, details: { count: bicepFiles.length, files: bicepFiles } });
          for (const file of bicepFiles) {
            try {
              const content = await this.githubClient.getFileContent(repoInfo.owner, repoInfo.repo, file);
              for (const resource of config.bicepChecks.requiredResources) {
                if (!content.includes(resource)) {
                  issues.push({ id: `bicep-missing-${resource.toLowerCase()}`, severity: "error", message: `Missing resource "${resource}" in ${file}`, error: `File ${file} does not contain required resource ${resource}` });
                } else {
                  compliant.push({ id: `bicep-resource-${resource.toLowerCase()}-${file}`, category: "bicepResource", message: `Found required resource "${resource}" in ${file}`, details: { resource, file } });
                }
              }
              this.analyzeAuthenticationMethods(content, file, issues, compliant);
            } catch (err) {
              console.error(`Failed to read Bicep file: ${file}`);
              issues.push({ id: `error-reading-${file}`, severity: "warning", message: `Failed to read ${file}`, error: err instanceof Error ? err.message : String(err) });
            }
          }
        }
        const azureYamlPath = files.find((f) => f === "azure.yaml" || f === "azure.yml");
        if (azureYamlPath) {
          compliant.push({ id: "azure-yaml-exists", category: "azureYaml", message: `Found azure.yaml file: ${azureYamlPath}`, details: { fileName: azureYamlPath } });
          try {
            const azureYamlContent = await this.githubClient.getFileContent(repoInfo.owner, repoInfo.repo, azureYamlPath);
            const azureSnippet = azureYamlContent.split("\n").slice(0, 120).join("\n");
            if (config.azureYamlRules?.mustDefineServices && !/services\s*:/i.test(azureYamlContent)) {
              issues.push({ id: "azure-yaml-missing-services", severity: "error", message: `No "services:" defined in ${azureYamlPath}`, error: `File ${azureYamlPath} does not define required "services:" section`, filePath: azureYamlPath, snippet: azureSnippet });
            } else if (config.azureYamlRules?.mustDefineServices) {
              compliant.push({ id: "azure-yaml-services-defined", category: "azureYaml", message: `"services:" section found in ${azureYamlPath}`, details: { fileName: azureYamlPath } });
            }
          } catch {
            issues.push({ id: "azure-yaml-read-error", severity: "warning", message: `Could not read ${azureYamlPath}`, error: `Failed to read file ${azureYamlPath}` });
          }
        } else {
          issues.push({ id: "missing-azure-yaml", severity: "error", message: "Missing azure.yaml or azure.yml file", error: "No azure.yaml or azure.yml file found in repository" });
        }
        ...
        const summary = issues.length === 0 ? "No issues found \u{1F389}" : "Issues found";
        ...
        compliant.push({ id: "compliance-summary", category: "meta", message: `Compliance: ${percentageCompliant}%`, details: { issueCount: issues.length, compliantCount: compliant.length, totalChecks, percentageCompliant } });
        const enrichedIssues = issues.map((i) => {
          try {
            ...
            if (v) {
              ...
              i.issueTemplate = formatViolationAsIssue(v, { compliancePercentage: percentageCompliant });
            }
          } catch (e) {
          }
          return i;
        });
        const result = { repoUrl, ruleSet, timestamp: (/* @__PURE__ */ new Date()).toISOString(), compliance: { issues: enrichedIssues, compliant, summary: `${summary} - Compliance: ${percentageCompliant}%` } };
        if (ruleSet === "custom" && customConfig) {
          result.customConfig = { gistUrl: customConfig.gistUrl || null };
        }
        return result;
      } catch (error) {
    ...
    async analyzeTemplateServerSide(repoUrl, ruleSetOrOptions) {
      this.debug("analyzer", `Using server-side analysis for ${repoUrl} with ruleset: ${ruleSetOrOptions}`);
      try {
        try {
          const w = window;
          ...
          if (repoMatchEarly) {
            ...
            w.__tdForkedRepos = w.__tdForkedRepos || /* @__PURE__ */ new Set();
            ...
            if (!already) {
              ...
              try {
                ...
                console.warn("[fork-preflight] fork attempt error (continuing):", forkErr?.message || forkErr);
                w.__tdForkLog.push({ phase: "error", repo: earlyKey, ts: Date.now(), error: forkErr?.message || String(forkErr) });
              }
            } else {
          ...
          if (isForceRescan) {
            ...
            if (repoMatchMU) {
              ...
              if (w.__tdForkedRepos && w.__tdForkedRepos.has(key)) {
                ...
                fetch(mergeUrl, { method: "POST", headers: { "Accept": "application/vnd.github+json" } }).then((r) => console.log("[force-rescan] merge-upstream status", r.status)).catch((e) => console.warn("[force-rescan] merge-upstream error", e?.message || e));
              }
            }
          }
        } catch (muErr) {
          ...
          if (repoMatch) {
            ...
            if (!w.__tdForkedRepos.has(key)) {
              ...
              try {
                ...
                console.warn("[analyzer][fork-preflight] fork attempt failed (continuing):", forkErr?.message || forkErr);
              }
            }
          }
        } catch (pfErr) {
        ...
        let endpoint = window.ApiRoutes ? window.ApiRoutes.build("analyze-template") : `${apiBase.replace(/\/$/, "")}/api/v4/analyze-template`;
        ...
        if (!response.ok) {
          ...
          if (/saml/i.test(errorText) || /sso/i.test(errorText) || response.status === 403) {
            try {
              document.dispatchEvent(new CustomEvent("analysis-saml-blocked", { detail: { repoUrl, status: response.status, body: errorText } }));
            } catch (_) {
            }
          }
          if ((response.status === 501 || response.status === 404) && (navigator.userAgent.includes("Playwright") || window.PLAYWRIGHT_TEST)) {
            console.warn("[analyzer] Backend unavailable (", response.status, ") returning synthetic stub result");
            return { repoUrl, ruleSet, timestamp: (/* @__PURE__ */ new Date()).toISOString(), compliance: { issues: [], compliant: [{ id: "synthetic", category: "meta", message: "Synthetic empty analysis", details: {} }], summary: "Synthetic (backend unavailable)" } };
          }
          throw new Error(`Server-side analysis failed: ${response.status} ${response.statusText} - ${errorText}`);
        }
        const result = await response.json();
        if (!result.timestamp) {
          result.timestamp = (/* @__PURE__ */ new Date()).toISOString();
        }
        return result;
      } catch (error) {
    ...
    evaluateDefaultBranchRule(config, repoInfo, defaultBranch, issues, compliant) {
      ...
      if (normalize(defaultBranch) !== normalize(expected)) {
        issues.push({ id: `default-branch-not-${expected}`, severity: "error", message: `Default branch must be '${expected}'. Current default branch is '${defaultBranch}'.`, error: `Default branch is '${defaultBranch}', expected '${expected}'` });
      } else {
        compliant.push({ id: `default-branch-is-${expected}`, category: "branch", message: `Default branch is '${expected}'`, details: { defaultBranch } });
      }
    }
    analyzeAuthenticationMethods(content, file, issues, compliant) {
      ...
      if (hasManagedIdentity) {
        compliant.push({ id: `bicep-uses-managed-identity-${file}`, category: "bicepSecurity", message: `Good practice: ${file} uses Managed Identity for Azure authentication`, details: { file, authMethod: "ManagedIdentity" } });
      }
      if (securityChecks.detectInsecureAuth && authMethods.length > 0) {
        const authMethodsList = authMethods.join(", ");
        issues.push({ id: `bicep-alternative-auth-${file}`, severity: "warning", message: `Security recommendation: Replace ${authMethodsList} with Managed Identity in ${file}`, error: `File ${file} uses ${authMethodsList} for authentication instead of Managed Identity`, recommendation: `Consider replacing ${authMethodsList} with Managed Identity for better security.` });
      }
    ...
    detectAuthenticationMethods(content) {
      ...
      if (/accessKey/i.test(content) || /['"]accessKey['"]/i.test(content) || /primaryKey/i.test(content) || /['"]primaryKey['"]/i.test(content) || /secondaryKey/i.test(content) || /['"]secondaryKey['"]/i.test(content)) {
      ...
      if (/sasToken/i.test(content) || /['"]sasToken['"]/i.test(content) || /sharedAccessSignature/i.test(content) || /SharedAccessKey/i.test(content)) {
      ...
      if (/AccountKey=/i.test(content) || /Password=/i.test(content) || /UserName=/i.test(content) || /AccountEndpoint=/i.test(content)) {
    ...
    validateRepoConfiguration(config, repoInfo, defaultBranch, issues, compliant) {
      try {
        ...
        console.error("Error validating repository configuration:", err);
        issues.push({ id: "repo-configuration-validation-failed", severity: "warning", message: "Repository configuration validation failed", error: err instanceof Error ? err.message : String(err) });
      }
    }
  };
  var existingTA = window.TemplateAnalyzer;
  if (!existingTA || typeof existingTA !== "object" || !("analyzeTemplateServerSide" in existingTA)) {
    window.TemplateAnalyzer = new TemplateAnalyzer();
  ...
  } else if (existingTA && typeof existingTA === "object" && typeof existingTA.analyzeTemplateServerSide !== "function") {
    existingTA.analyzeTemplateServerSide = function(repoUrl, ruleSet) {
  ...
  window.checkAnalyzerReady = function() {
    return !!window.TemplateAnalyzer;
  };
  if (!window.analyzeRepository) {
    window.analyzeRepository = function(repoUrl, ruleSet) {
      const ta = window.TemplateAnalyzer;
...
//# sourceMappingURL=analyzer.bundle.js.map
