...
export default wrapHttp(async (req: any, ctx: Context, requestId: string) => {
  ...
  if (!targetRepoUrl || typeof targetRepoUrl !== 'string' || !/^https?:\/\//i.test(targetRepoUrl)) {
    return { status: 400, body: { error: 'targetRepoUrl is required and must be a valid URL', requestId } };
  }
  ...
  ctx.log('validation-template dispatch', { requestId, ghUrl, branch, workflowFile });
  try {
    ...
    if (!res.ok) {
      ...
      ctx.log.error('validation-template dispatch failure', { requestId, status: res.status, text });
      return { status: 502, body: { error: `GitHub dispatch failed: ${res.status} ${res.statusText}`, details: text.slice(0,1000), requestId } };
    }
    return { status: 200, body: { runId, message: 'Workflow triggered', requestId } };
  } catch (err: any) {
    ctx.log.error('validation-template exception', { requestId, error: err?.message });
    return { status: 500, body: { error: 'Internal dispatch error', requestId } };
  }
});