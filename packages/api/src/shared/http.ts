...
export function wrapHttp(handler: SimpleHandler): (ctx: Context, req: HttpRequest) => Promise<any> {
  ...
  const origin = (req.headers && (req.headers['origin'] || req.headers['Origin'])) || '';
    ...
    try {
      ...
      ctx.res = { status: 500, headers: baseHeaders, body: { error: 'Internal Server Error', requestId } } as any;
      return ctx.res;
    }
  };
}
