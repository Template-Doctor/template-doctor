...
function createGitHubHeaders() {
    ...
    if (!workflowToken)
        throw new Error("Missing GH_WORKFLOW_TOKEN app setting");
...
async function getRepoScanArtifactUrl(artifacts) {
    const repoArtifact = artifacts.find(
        (a) => typeof a.name === "string" && a.name.startsWith("scan-repo-"),
    );
    if (!repoArtifact) {
...
async function getImageArtifactUrls(artifacts) {
    const imageArtifacts = artifacts.filter(
        (a) => typeof a.name === "string" && a.name.startsWith("scan-image-"),
    );
        ...
        if (
            ...
            if (!fileResp.ok) {
                ...
                const error = new Error(
                    `Failed to download artifact from storage: ${fileResp.status} ${fileResp.statusText}`,
                );
                throw error;
            }
            ...
            throw new Error(
                `Failed to get artifact zip from download URL: ${response.status} ${response.statusText}`,
            );
        } else {
            if (response.ok) {
                ...
                throw new Error(
                    `Unexpected response when fetching artifact: ${response.status} ${response.statusText}`,
                );
            }
        }
    } catch (err) {
...
async function processRepoArtifact(
    context,
    artifacts,
    correlationId = null,
    ...
    const repoScanJsonResult = await extractTrivyResults(
        context,
        buffer,
        correlationId,
    );

    const repoTrivyResults = processTrivyResultsDetails(
        repoScanJsonResult,
        includeAllDetails,
    );
...
async function processImageArtifact(
    ...
    if (context && context.log) {
        context.log(
            `Processing ${imageUrls.length} image artifacts in chunks of ${chunkSize}`,
            {
    ...
    for (let i = 0; i < imageUrls.length; i += chunkSize) {
        ...
        if (context && context.log) {
            context.log(
                `Processing image chunk ${Math.floor(i / chunkSize) + 1}/${Math.ceil(imageUrls.length / chunkSize)}`,
                {
                    operation: "processImageArtifact",
                    chunkStart: i,
        ...
        try {
            ...
            const downloadPromises = chunk.map((url) =>
                getZipAsBuffer(url, context),
            );
            const buffers = await Promise.all(downloadPromises);

            // Extract and process each buffer in this chunk
            const extractPromises = buffers.map((buf) =>
                extractTrivyResults(context, buf, correlationId),
            );
            const chunkJsonResults = await Promise.all(extractPromises);

            // Process details for each result in this chunk
            const chunkProcessedResults = chunkJsonResults.map((r) =>
                processTrivyResultsDetails(r, includeAllDetails),
            );
            ...
            if (context && context.log) {
                context.log(
                    `Completed processing image chunk ${Math.floor(i / chunkSize) + 1}`,
                    {
                        operation: "processImageArtifact",
            ...
            if (context && context.log && context.log.error) {
                context.log.error(
                    `Error processing image chunk ${Math.floor(i / chunkSize) + 1}`,
                    {
                        operation: "processImageArtifact",
                        error: err.message,
                        stack: err.stack,
...
async function processArtifacts(
    ...
    const artifacts =
        artifactsObj && artifactsObj.artifacts ? artifactsObj.artifacts : null;
    ...
    // Repo scan
    const repoScanResult = await processRepoArtifact(
        context,
        artifacts,
        correlationId,
        includeAllDetails,
    ...
    // Images
    const imageScanResults = await processImageArtifact(
        context,
        artifacts,
        correlationId,
        includeAllDetails,
    ...
    if (repoScanResult) {
        ...
        if (repoScanResult.criticalVulns > 0) {
            addIssue(
                issues,
                "docker-repo-critical-vulnerabilities",
                "error",
        ...
        if (repoScanResult.highVulns > 0) {
            addIssue(
                issues,
                "docker-repo-high-vulnerabilities",
                "warning",
        ...
        if (repoScanResult.criticalMisconfigurations > 0) {
            addIssue(
                issues,
                "docker-repo-critical-misconfigurations",
                "error",
    ...
    if (imageScanResults && imageScanResults.length > 0) {
        ...
        imageScanResults.forEach((imageScan, index) => {
            ...
            if (imageScan.criticalVulns > 0) {
                ...
                addIssue(
                    issues,
                    `docker-image-${index}-critical-vulnerabilities`,
                    "error",
                    `Docker image ${imageScan.artifactName || `#${index + 1}`} contains ${imageScan.criticalVulns} critical vulnerabilities`,
                    {
                        count: imageScan.criticalVulns,
                        artifactName: imageScan.artifactName,
                        repository: imageScan.repository,
                        tag: imageScan.tag,
                    },
                );
            }
        ...
        if (!hasCriticalVulns) {
            compliance.push({
                ...
                category: "security",
                message:
                    "All Docker images are free of critical vulnerabilities",
        ...
        if (!hasCriticalMisconfigs) {
            compliance.push({
                ...
                category: "security",
                message:
                    "All Docker images are free of critical misconfigurations",
                details: { imageCount: imageScanResults.length },
            });
        }
...
module.exports = async function (context, req) {
    ...
    try {
        ...
        const includeAllDetails =
            req.body && req.body.includeAllDetails === true ? true : false;
        ...
        if (triggerResult.status !== 200 || !triggerResult.found) {
            ...
            addIssue(
                issues,
                "docker-workflow-trigger-failed",
                "error",
        ...
        const artifacts = await getArtifactsForRun(
            workflowOwner,
            workflowRepo,
            runId,
            context,
        ...
        const complianceResults = await processArtifacts(
            context,
            artifacts,
            correlationId,
            includeAllDetails,
        ...
        context.res = {
            ...
            body: {
                ...
                githubRunUrl: runId
                    ? `https://github.com/${workflowOwner}/${workflowRepo}/actions/runs/${runId}`
                    : null,
        ...
        context.log(
            `Trivy validation completed for ${templateUrl} with local run ID: ${runId}: ${JSON.stringify(context.res.body)}`,
        );
    } catch (err) {
        context.log.error({
        ...
        const issues = [
            {
                ...
                message:
                    err.message ||
                    "Unknown error occurred during Docker image validation",
                details: {
