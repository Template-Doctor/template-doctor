...
export default wrapHttp(async (req: any, ctx: Context, requestId: string) => {
  ...
  const body: CallbackBody = (req.body && typeof req.body === 'object') ? req.body : {};
  const { runId, githubRunId, status, result } = body;
  if (!runId || !githubRunId) {
    return { status: 400, body: { error: 'runId and githubRunId are required', requestId } };
  }
  ...
  ctx.log('validation-callback received', { runId, githubRunId, status, requestId });
