...
export async function enableBackendMigration(page) {
  await page.addInitScript(() => {
    try { localStorage.setItem('td_backend_migration', '1'); } catch {}
...
export async function ensureApiClientReady(page) {
  ...
  await page.waitForFunction(() => !!(window.ApiRoutes || (window.GitHubClient && window.GitHubClient.auth)), { timeout: 5000 });
}
