...
describe("Authentication Middleware", () => {
    ...
    describe("requireAuth", () => {
        ...
        it("should return 401 when GitHub token is invalid", async () => {
            ...
            (mockOctokit.users.getAuthenticated as any).mockRejectedValue(
        ...
        it("should attach user info and call next() when token is valid", async () => {
            ...
            const { Octokit } = await import("@octokit/rest");
            const mockOctokit = new Octokit({ auth: "valid_token" });
            (mockOctokit.users.getAuthenticated as any).mockResolvedValue({
                data: mockUserData,
            });
