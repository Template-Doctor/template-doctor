...
async function fetchArtifacts(owner: string, repo: string, runId: string | number, token: string, ctx: Context) {
  ...
  if (!res.ok) {
    ...
    ctx.log.error('action-run-artifacts: fetch failed', { status: res.status, text: text.slice(0,500) });
  ...
  try {
    ...
    return {
      ...
      body: {
        error: null,
        data: result.json,
        context: { ownerRepo: `${owner}/${repo}`, requestId, workflowRunId, artifactCount }
      }
    };
  } catch (err: any) {
    ctx.log.error('action-run-artifacts: unexpected error', { requestId, error: err?.message });
    return { status: 500, body: { error: 'Internal Server Error', errorType: 'SERVER_ERROR', requestId } };
  }
});
